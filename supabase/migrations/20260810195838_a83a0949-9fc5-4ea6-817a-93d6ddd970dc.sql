...
WITH CHECK (
    ...
    AND (
        public.has_role(auth.uid(), tenant_id, 'school_admin'::public.app_role) OR 
        public.has_role(auth.uid(), tenant_id, 'school_owner'::public.app_role) OR 
        public.has_role(auth.uid(), tenant_id, 'platform_admin'::public.app_role)
    )
);

-- 2. Audit Log Security Refinement
